...
const BattleAndResult = ({ close, resetLoading, explanation, finished }) => {
  ...
  const [currentState, setCurrentState] = useState(State.BATTLE);

  useEffect(() => {
    if (practiceData) {
      // Start the battle
      const battleTimer = setTimeout(() => {
        setCurrentState(State.RESULT); // Move to result state

        // After 1 second, move to explanation state
        const resultTimer = setTimeout(() => {
          setCurrentState(State.EXPLANATION);
          explanation(true); // Assuming explanation(true) is a prop function to show explanation
        }, 1000);
    ...
    <>
      <div>
        <div>
          ...
          {/* Assuming the explanation UI is managed outside or via the explanation prop function */}
        </div>
      </div>
    </>
  );
...
export default function page() {
  ...
  const [battleResultState, setBattleResultState] = useState(false);
  ...
  return (
    <>
      ...
      {battleResultState && (
        <BattleAndResult
          close={setBattleResultState}
          resetLoading={resetLoadingPrep}
          finished={setFinished}
          explanation={setExplanationState}
        />
      )}
      {bktInfo ? (
        <div>
